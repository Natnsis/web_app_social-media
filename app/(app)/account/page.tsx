...
import {
    Gear, Globe, ArrowRightFromBracket, Heart, File, Bookmark, Radio, User,
...
export default function AccountPage() {
    ...
    return (
        <div className="relative flex h-full flex-col overflow-hidden">
            ...
            <header className="flex shrink-0 items-center justify-end px-4 py-2 lg:hidden">
            ...
            <div className="flex-1 overflow-y-auto pb-24">
                <div className="lg:mx-auto lg:flex lg:w-full lg:max-w-5xl lg:gap-8 lg:px-8 lg:pt-8">
                    {/* Desktop sidebar card */}
                    <aside className="lg:flex lg:w-72 lg:shrink-0 lg:flex-col">
                        <div className="lg:rounded-2xl lg:border lg:border-border lg:bg-card lg:p-6">
                            ...
                            <div className="mt-5 hidden space-y-2 lg:block">
                                <Link href="/account/settings">
                                    <Button variant="outline" className="w-full justify-start gap-3 rounded-xl" size="sm">
                                        <Gear size={16} /> Settings
                                    </Button>
                                </Link>
                                <Button variant="ghost" className="w-full justify-start gap-3 rounded-xl text-destructive hover:bg-destructive/10" size="sm"
                                    onClick={() => { logout(); router.push("/login") }}>
                    ...
                    <div className="flex-1 min-w-0 lg:mt-0">
                        ...
                        <div className="hidden lg:mb-6 lg:block">
                            <h1 className="text-2xl font-black tracking-tight">{user?.org ?? "Beza International"}</h1>
                            <p className="text-sm text-muted-foreground">{user?.name ?? "Abebe Tesfaye"} &middot; {user?.role ?? "Global Administrator"}</p>
                        </div>

                        {/* Tabs */}
                        <Tabs defaultValue="Posts">
                            <TabsList className="flex w-full bg-transparent border-b border-border rounded-none px-4 h-auto pb-0 gap-0 lg:px-0">
                                {[
                                    ...
                                    <TabsTrigger key={tab} value={tab}
                                        className="flex-1 rounded-none border-b-2 border-transparent pb-2.5 pt-1 text-sm font-semibold data-active:border-primary data-active:text-primary data-active:bg-transparent lg:flex-none lg:px-4">
                            ...
                            {["Posts", "Saved", "Liked", "Channels"].map((tab) => (
                                <TabsContent key={tab} value={tab}>
                                    ...
                                    {/* Daily verse */}
                                    <div className="mx-4 mb-4 overflow-hidden rounded-2xl lg:mx-0">
